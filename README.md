# mac-keys
mini repo of a [german] keyboard config file to use [german ISO] win keyboards on mac OS

## Usage
1. Download the file
2. Go to: [Your Macintosh HD]/Library/Keyborard Layouts
3. Drop the file in there
4. Go to: [System Settings] / Keybord / Text Input / Input Sources -> Edit...
5. Use the [ + ] Button to add another Layout
6. Search for others and select the profile, or search for SWAP where you find "Win DE SWAP FIX", use that, click add.
7. Optional: Use "Show Input menu in menu bar" to quickly change between layouts, helpful for Macbooks. 

If someone codes a flesh eating computer virus with that or anything similar, I will not be held accountable. <see license>
